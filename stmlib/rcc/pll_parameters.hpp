...
namespace rcc
{
    ...
    namespace detail
    {
        ...
        {
            return (value == 2) ? 1 : ((value > 2) ? (1 + log2_floor_float(value / 2)) : 0);
        }

        constexpr int log2_ceil_float(double value)
        {
            return (value == 2) ? 1 : ((value > 2) ? (1 + log2_ceil_float(value / 2)) : 1);
        }

        constexpr int log2_floor(double value)
        {
            return (value == 1) ? 0 : ((value >= 1) ? log2_floor_float(value) : (-log2_ceil_float(1. / value)));
        }

        constexpr int log2_ceil(double value)
        {
            return (value == 1) ? 0 : ((value > 1) ? log2_ceil_float(value) : (-log2_floor_float(1. / value)));
        }
        ...
        {
            return (value > 1) ? next_power_of_two(value >> 1, shifts + 1) : (1 << (shifts + 1));
        }

        constexpr int prior_power_of_two(int value, int shifts = 0)
        {
            return (value > 1) ? prior_power_of_two(value >> 1, shifts + 1) : (1 << (shifts));
        }
        ...
        struct sol
        {
            constexpr sol(int n_, int m_, int p_, int a_) : n(n_), m(m_), p(p_), a(a_)
            {
            }
        ...
        {
            return myabs(eval(a, source) - dest) < myabs(eval(b, source) - dest) ? a : b;
        }
        ...
        constexpr sol p_search_pruned(double source, double dest, int m, int a, int p, int pmax, sol best)
        {
            return (p > pmax) ? best :
                       ...
                       p_search_pruned(source, dest, m, a, p + 2, pmax,
                                       better_sol(source, dest, best, n_search(source, dest, m, a, p, best)));
        }
        ...
        constexpr int encode_a(int a)
        {
            ...
            return (a == 1) ? 0 : 8 | ((a <= 16) ? log2_ceil(a) : (log2_ceil(a) - 1));
        }
        ...
        constexpr sol a_search_pruned(double source, double dest, int m, int a, int amax, sol best)
        {
            return (a > 512) ? best :
                       ...
                       a_search_pruned(source, dest, m, next_a(a), amax,
                                       better_sol(source, dest, best, p_search(source, dest, m, a, best)));
        }
