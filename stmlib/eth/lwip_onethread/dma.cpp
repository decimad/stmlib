...
// Running on Cortex M7 make sure the buffers are in uncached memory
...
//	__attribute__ ((section(".dma")))
    ...
    void rx_walk_descriptors(struct netif *interface)
    {
        ...
        while (!rx_descriptor_ring.empty() && rx_descriptor_ring.committed_front()->is_host())
        {
            ...
            rx_descriptor_recycle(); // this must work in 100% of all cases concurrently to the pbuf_free function, so I
