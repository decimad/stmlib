...
namespace eth::lwip
{
    ...
    void start_mac_dma(uint16 phystats, struct netif &interface)
    {
        ...
        eth::device.dmardlar = reinterpret_cast<uint32>(first_armed);

        eth::device.dmatdlar = reinterpret_cast<uint32>(tx_front);
    ...
    const uint8 mac_addr[6] = {0x00, 0xFA, 0x5F, 0x48, 0x59, 0x66};
