...
namespace util
{
    ...
    struct check_alignment<unsigned int, address>
    {
        static_assert((address & 3) == 0, "misaligned access!");
    };
    template <unsigned int address>
    struct check_alignment<int, address>
    {
        static_assert((address & 3) == 0, "misaligned access!");
    };
    template <unsigned int address>
    struct check_alignment<unsigned short, address>
    {
        static_assert((address & 1) == 0, "misaligned access!");
    };
    template <unsigned int address>
    struct check_alignment<short int, address>
    {
        static_assert((address & 1) == 0, "misaligned access!");
    };
    template <unsigned int address>
    struct check_alignment<unsigned char, address>
    {
    };
    template <unsigned int address>
    struct check_alignment<signed char, address>
    {
    };
...
namespace bit
{
    ...
    struct make_mask
    {
        static const unsigned int value = (1 << Bits) - 1; // (make_mask< Bits-1 >::value << 1) | 1;
    };
    ...
    struct masked_value
    {
        constexpr masked_value(T value, T mask) : value_(value), mask_(mask)
        {
        }
            ...
            using type = typename std::conditional<
                ...
                typename std::conditional<
                    ...
                    typename std::conditional<(Width <= 32), uint32,
                                              typename std::conditional<(Width <= 64), uint64, void>::type>::type>::type>::type;
        };
    ...
    namespace optimized
    {
        ...
        struct inspect_mask<T, mask, pos, numbits, bit_list<bits...>>
            : public std::conditional<(mask & 1) == 1,
                                      inspect_mask<T, (mask >> 1), pos - 1, numbits + 1, bit_list<bits..., sizeof(T) * 8 - pos>>,
                                      inspect_mask<T, (mask >> 1), pos - 1, numbits, bit_list<bits...>>>::type
        {
        };
         ...
         */
        template <typename T, uint32 Address, T Mask, uint8 Bitcount = inspect_mask<T, Mask>::bitcount,
                  typename BitList = typename inspect_mask<T, Mask>::bitlist>
        ...
        {
            constexpr static_masked_value(T value) : value_(value)
            {
            }
        ...
        constexpr static_masked_value<T, lhs_mask | rhs_mask> operator|(static_masked_value<T, lhs_mask> lhs,
                                                                        static_masked_value<T, rhs_mask> rhs)
        {
    ...
    template <unsigned int Width, unsigned int Count, unsigned int Mask = (unsigned int)-1>
    constexpr typename detail::TypeFromWidth<Width * Count>::type replicate(unsigned int value)
    {
        return detail::replicate_bits<typename detail::TypeFromWidth<Width * Count>::type, Width, Count, Mask>::exec(make_mask<Width>::value
                                                                                                                     & value);
    }
    ...
    {
        return masked_value<typename detail::TypeFromWidth<Width * Count>::type>(replicate<Width, Count, Mask>(value),
    ...
    struct register_snapshot
    {
        ...
        register_snapshot(T value) : value_(value)
        {
        }
