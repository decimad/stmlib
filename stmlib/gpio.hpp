...
namespace gpio {
	...
	constexpr inline reg_detail::gpio_regs& port_ref(uint8 port) {
		return *reinterpret_cast<reg_detail::gpio_regs*>(0x40020000 + port*0x400);
	}

	static auto& porta = port_ref(0);
	static auto& portb = port_ref(1);
	static auto& portc = port_ref(2);
	static auto& portd = port_ref(3);
	static auto& porte = port_ref(4);
	static auto& portf = port_ref(5);
	static auto& portg = port_ref(6);
	static auto& porth = port_ref(7);
	static auto& porti = port_ref(8);
	...
	public:
		...
		constexpr pin_ref( const static_pin<Port,Pin>& ) :
			port_(Port), pin_(Pin)
		{}

		constexpr pin_ref(uint8 port__, uint8 pin__)
			: port_(port__), pin_(pin__)
		{
		}
	...
	struct alternate_output {
		constexpr alternate_output( AlternateMode alternate_mode_, OutputType output_type_ = OutputType::PushPull, Speed speed_ = Speed::High )
		...
		static void run_dynamic( const pin_ref& p, const alternate_output& config) {
			...
			const auto alternate = bit::masked_value<uint64>(static_cast<uint8>(config.alternate_mode), bit::make_mask<4>::value) << (p.index()*4);
			port.afrl <<= alternate;
			port.afrh <<= alternate >> 32;
	...
	struct alternate_input {
		constexpr alternate_input( AlternateMode alternate_mode_ )
			: alternate_mode(alternate_mode_) {}
	...
	struct output {
		constexpr output( Speed speed_ = Speed::High )
			: speed(speed_) {}
		...
		static void run_dynamic( const pin_ref& p, const output& config )
		{
			...
			port.otyper  <<= bit::masked_value<uint32>(static_cast<uint8>(OutputType::PushPull), bit::make_mask<1>::value) << (p.index()*1);
			port.ospeedr <<= bit::masked_value<uint32>(static_cast<uint8>(config.speed), bit::make_mask<2>::value) << (p.index()*2);
		}
	...
	struct inout {
		constexpr inout( Speed speed_ = Speed::High )
			: speed(speed_) {}
		...
		static void run_dynamic( const pin_ref& p, const inout& config )
		{
			...
			port.moder   <<= bit::masked_value<uint32>(1, bit::make_mask<2>::value) << (p.index()*2);
			port.otyper  <<= bit::masked_value<uint32>(static_cast<uint8>(OutputType::OpenDrain), bit::make_mask<1>::value) << (p.index()*1);
			port.ospeedr <<= bit::masked_value<uint32>(static_cast<uint8>(config.speed), bit::make_mask<2>::value) << (p.index()*2);
		}
